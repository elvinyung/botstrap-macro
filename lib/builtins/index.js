...
exports.empty = function(cb, args) {
  cb(null, '');
};

const ID_REGEX = /\<[@#](\w+)|.*>/;
exports.equal = function(cb, args) {
  // fuck, I have to do type inference

  let valueEquals = (a, b) => {
    let aVal = a.match(ID_REGEX) ? a.match(ID_REGEX)[1] : a;
    let bVal = b.match(ID_REGEX) ? b.match(ID_REGEX)[1] : b;
    return aVal === bVal;
  };

  cb(null, args.reduce((a, b) => valueEquals(a, b)) ? '1' : '');
};
...
exports.replace = function(cb, args) {
  ...
  try {
    ...
    var result = text.replace(matcher, replacement);
    cb(null, result);
  }
  catch (e) {
    cb(e);
