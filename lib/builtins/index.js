...
exports.randexp = function(cb, args) {
  ...
  try {
    ...
    randexp.randInt = (from, to) => RandExp.prototype.randInt(
      ensureBounds(from, REP_LOWER_BOUND, REP_UPPER_BOUND),
      ensureBounds(to, REP_LOWER_BOUND, REP_UPPER_BOUND)
    );
    let result = randexp.gen();

    if (result.length > 4000) {
      throw new Error(`Bad regex ${pattern}`);
    }
