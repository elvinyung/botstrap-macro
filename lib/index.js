...
var baseApplyMacro = function baseApplyMacro(callee, args, cb, callStack) {
  ...
  if (typeof template === 'function') {
    ...
    var tokens = tokenizer.tokenizeExpr(resultSrc);
...
var interpretExpr = function interpretExpr(expr, cb) {
  var tokens = tokenizer.tokenizeExpr(expr);
...
var macro = function macro(argv, message, response, config, logger) {
  ...
  if (subcmd === 'set') {
    ...
    addMacro(name, template, function(err) {
      if (!err) {
        ...
        if (template.length >= 0) {
          ...
          respBody += lfmt.format('Successfully removed macro {{name}}', {
            name: name
          });
        }
    ...
    var callee = subcmd;
    var tokens = tokenizer.tokenizeExpr(argv.slice(2).join(' '));
